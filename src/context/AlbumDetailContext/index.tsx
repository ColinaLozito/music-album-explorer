 ...
 */
const AlbumDetailContext = createContext<AlbumDetailContextType | undefined>(
  undefined,
);
...
export const AlbumDetailProvider = ({ children }: { children: ReactNode }) => {
  const [albumDetail, setAlbumDetailState] = useState<AlbumDetails | null>(
    null,
  );
  ...
  return (
    <AlbumDetailContext.Provider
      value={{ albumDetail, setAlbumDetail, clearAlbumDetail }}
    >
      {children}
    </AlbumDetailContext.Provider>
  );
...
export const useAlbumDetail = () => {
  ...
  if (!ctx)
    throw new Error(
      "useAlbumDetail must be used within an AlbumDetailProvider",
    );
  return ctx;
};
